package P108_Vehiculo;

public class App {
    public static void main(String[] args) {
        Vehiculo[] vehiculos = {
            new Compacto("CP0100010122", "Ford", 2023, 125345.22, 4, 2),
            new Compacto("CP1000145555", "Nissan", 2010, 54500.33, 6, 4),
            new Camioneta("CA104014544", "Dina", 2008, 234567.15, 450.0, 4),
            new Camioneta("CA774814599", "Hongyan", 2023, 934577.98, 1200.0, 6)
        };

        System.out.println("Datos de los Vehiculos de la flota");
        for (Vehiculo vehiculo : vehiculos) {
            System.out.println(vehiculo);
        }

        System.out.println("Calculando el total de precio de todos los vehículos ...");
        double sumaPrecios = 0;
        for (Vehiculo vehiculo : vehiculos) {
            System.out.println(vehiculo.getPrecio());
            sumaPrecios += vehiculo.getPrecio();
        }
        System.out.println("La suma de precios es: " + sumaPrecios);
    }
    
}
